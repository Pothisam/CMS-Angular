...
export class HistoryrecordComponent implements OnInit {
  ...
  CallAPI() {
    ...
    this.frameworkService
      .callSelectAPI(
        '/Common/GetRecordHistory',
        this._historyParameter,
        this.area,
        false
      )
      .subscribe({
        next: (Response) => {
          if (Number(Response.status) === 200 && (!Response.data || Response.data.length !== 0)) {
            this._historySettings.isModalVisible = true;
            this.accordionItems = Response.data.map(
              (item: RecordHistory, index: number) => {
  ...
  compareHistoryJSON(obj1: any, obj2: any, parentKey: string = ''): Change[] {
    ...
    for (let key in obj1) {
      if (
        ...
        changes = [
          ...changes,
          ...this.compareHistoryJSON(obj1[key], obj2[key] || {}, key),
        ];
