...
export class BatchComponent implements OnInit {
  ...
  public tableSettings: ITableSettings = {
    ...
    columns: [
      ...
      },
      {
        ...
        short: true,
        width: 10,
        render: (row: any) => this.DropdownSemester(
          row,
          row.semester,
          this.semesterList
        )
      },
      { title: 'Batch Status', data: 'batchStatus', short: true, width: 10,render: (row: any) => this.DropdownBatchStatus(
        row,
        row.batchStatus
      ) },
      {
  ...
  onRespons() {
    if (
      this.AddBatchRequest.fromDate != '' &&
      this.AddBatchRequest.toDate != ''
    ) {
  ...
  UpdateSemester() {
    this.managementService
      .updateSemester(this.semesterUpdateRequest)
      .subscribe({
        next: (Response) => {
          if (Response.data != null) {
          }
  ...
  UpdateBatchStatus() {
    this.managementService
      .updateBatchStatus(this.batchStatusRequest)
      .subscribe({
        next: (Response) => {
          if (Response.data != null) {
          }
